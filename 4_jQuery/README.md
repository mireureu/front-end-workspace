...
1. `$("element[attridute]")` : 해당 속성을 가지는 요소만 지정
2. `$("element[attridute='value']")` : 특정 요소 내에 속성과 값이 일치하는 요소 지정
3. `$("element[attridute~='value']")` : 특정 요소 내에 공백을 포함한 속성값이 일치하는 요소 지정
4. `$("element[attridute^='value']")` :  특정 요소 내에 지정한 속성값으로 시작하는 요소 지정
5. `$("element[attridute$='value']")` : 특정 요소 내에 지정한 속성값으로 끝나는 요소 지정  
6. `$("element[attridute*='value']")` : 특정 요소 내에 속성의 값을 포함하는 요소 지정
7. `$("element[attridute!='value]")` : 해당 속성값을 갖지 않는 요소만 지정
...
- `$('선택자').last()` : 선택된 요소들 중 마지먹번째 요소 선택
- `$('선택자').eq(n)` : 선택된 요소들 중 인덱스 번호와 일치하는 요소 선택 (0부터 시작)
- `$('선택자').filter('선택자')` : 선택된 요소들 중 인자값과 일치하는 요소 선택
- `$('선택자').odd()` : 홀수번쨰 인덱스에 위치한 요소 선택
- `$('선택자').even()` : 짝수번째 인덱스에 위치한 요소 선택
- `$('선택자').not()` : 선택된 요소들 중 특정 요소와 일치하지 않는 요소 선택
- `$('선택자').is(선택자)` : 선택된 요소들 중 특정 요소가 잇는지 확인(true, false를 리턴)
...
- `$('선택자').parentUntil(['선택자'])` : 선택된 요소의 모든 상위 요소를 선택하거나, 매개변수가 있으면 선택된 요소부터 매개변수와 일차하는 요소까지의 상위 요소 선택
...
- 클래스 선택자로 여러 요소가 선택되었을 떄 전체 요소의 텍스트를 리턴
...
- 마우스 이벤트 : **click**, ablclick, **mouesenter**, **mouseleave**, mousedown, mouseup, mouseover, mouseout, mousemove
